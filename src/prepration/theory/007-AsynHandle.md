  ...
  useEffect(() => {
    const fetchBalance = async () => {
      try {
        ...
        const response = await fetch("https://api.ledger.local/v1/balance");
    ...
    useCallback(() => {
      ...
      const loadData = async () => {
        try {
          ...
          const response = await fetch("https://api.com/dashboard", { signal });
...
function ScreenA() {
  const { data, isLoading } = useQuery({
    ...
    queryFn: async ({ signal }) => {
      const res = await fetch("https://api.com/dashboard", { signal });
